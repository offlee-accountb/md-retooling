...
## 2025-11-20 – Phase 1.5 표 보더/이중실선 불일치
...
- 여전히 Tier1 기준과 미세한 차이가 존재(오른쪽 열 하단/헤더 배경 일관성).

### To-Do / Next Steps
1. `validator/tier_test_inputmodel/(2-1)test_inputmodel.hwpx`와 최신 산출물의 `borderFill`(12~17) 정의와 셀 매핑을 행·열별로 비교.
2. 필요 시 borderFill 19+를 추가 정의해 오른쪽 열/마지막 행 전용 케이스를 분리.
3. 헤더 배경색 유지 여부 확인 후, 헤더 row에 강제 fillBrush 적용 옵션 검토.
4. 변경 후 Tier 테스트 + 한글 눈검으로 회귀 검증.
